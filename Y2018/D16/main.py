...
class Solver2018Day16(Solver):
    ...
    def rr_aux(self, reg, op, operation):
        if 0 <= min(op) and max(op) < 4:
            reg[op[2]] = int(operation(reg[op[0]], reg[op[1]]))
            return reg
        else:
            return []

    def ri_aux(self, reg, op, operation):
        if 0 <= op[0] < 4 and 0 <= op[2] < 4:
            reg[op[2]] = int(operation(reg[op[0]], op[1]))
            return reg
        else:
            return []

    def ir_aux(self, reg, op, operation):
        if 0 <= op[1] < 4 and 0 <= op[2] < 4:
            reg[op[2]] = int(operation(op[0], reg[op[1]]))
            return reg
        else:
            return []
    ...
    def __init__(self, src):
        ...
        self.ls, self.ops = ls, ops

    def solve_part_1(self):
        ans = 0
        for before, op, after in self.ls:
            cnt = 0
            for sim in self.op_list_run:
                if sim(self, before.copy(), op[1:]) == after:
                    cnt += 1
            if cnt >= 3:
                ans += 1
        return ans
    ...
    def solve_part_2(self):
        ...
        for before, op, after in self.ls:
            for sim in self.op_list_run:
                if sim(self, before.copy(), op[1:]) != after and sim in st_list[op[0]]:
