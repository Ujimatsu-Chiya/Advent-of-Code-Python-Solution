...
class Solver23D10(Solver):
    INPUT_URL = 'https://adventofcode.com/2023/day/10/input'
        ...
        for i in range(n):
            for j in range(m):
                if mp[i][j] == 'S':
                    sx, sy = i, j
        st = set()
        for dx, dy in Solver23D10.all_dirs:
            x, y = sx + dx, sy + dy
            if 0 <= x < n and 0 <= y < m:
                for ex, ey in Solver23D10.dirs[mp[x][y]]:
                    tx, ty = x + ex, y + ey
                    if tx == sx and ty == sy:
                        st.add((-ex, -ey))
        for k, v in Solver23D10.dirs.items():
            if v == st:
                mp[sx][sy] = k
        return mp, sx, sy
...
if __name__ == "__main__":
    ...
    src = requests.get(Solver23D10.INPUT_URL, cookies=cookies).text
